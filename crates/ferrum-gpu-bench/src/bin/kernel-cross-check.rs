...
fn check_specialised_256(
    ...
    let plan = Plan::new(log_n, batch, false);
    assert_eq!(plan.kernel_kind, KernelKind::Specialised256);
    ...
    let pass = rel <= REL_TOL;
    println!(
        "fft_c2c_256 vs CPU (N={n}, batch={batch}): max_rel_err={rel:.2e} at bin {idx} -> {}",
        if pass { "PASS" } else { "FAIL" }
    );
    Ok(pass)
}
...
fn main() -> Result<()> {
    ...
    all_pass &= check_specialised_256(&ctx, &module)?;
