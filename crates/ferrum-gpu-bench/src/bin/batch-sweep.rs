...
fn main() -> Result<()> {
    ...
    let wblock: u32 = std::env::var("WARP_BLOCK")
        .ok()
    ...
    // --- r16s correctness vs radix-2 (batch 4 so we exercise multiple blocks). ---
    {
        ...
        println!(
            "fft_c2c_256_r16s vs radix-2: max_rel_err = {worst:.2e} -> {}",
            if worst <= 1e-3 { "PASS" } else { "FAIL" }
        );
        if worst > 1e-3 {
            return Err(anyhow!("r16s correctness FAIL"));
        }
    }

    println!(
        "N=256 batch sweep  (ratio = ferrum/cufft, <1.0 BEATS cuFFT)  warp K={}",
        wblock / 32
    );
    println!(
        "{:<9} {:<9} {:<9} {:<9} {:<10} {:<12}",
        "batch", "warp", "radix4", "r16s", "cufft_us", "best"
    );

    for &b in &batches {
        let wcfg = LaunchConfig {
            grid_dim: ((b as u32 * 32) / wblock, 1, 1),
            block_dim: (wblock, 1, 1),
            shared_mem_bytes: 0,
        };
        let warp = |din: &DeviceBuffer<f32>,
                    _t: &DeviceBuffer<f32>,
                    dout: &mut DeviceBuffer<f32>|
         -> Result<()> {
            mod_w.fft_c2c_256_warp(stream.as_ref(), wcfg, din, &d_w32, &d_w256, dout)?;
            Ok(())
        };
        ...
        let r4 = |din: &DeviceBuffer<f32>,
                  t: &DeviceBuffer<f32>,
                  dout: &mut DeviceBuffer<f32>|
         -> Result<()> {
            mod_k.fft_c2c_256(stream.as_ref(), rcfg, din, t, dout)?;
            Ok(())
        };
        let (fr, _) = alternating_bench_batch(&core_ctx, &cudarc_ctx, 8, b, r4)?;
        let r4_r = (fr.event_med_s * 1e6 / b as f64) / cu_us;

        let rscfg = LaunchConfig {
            grid_dim: (b as u32, 1, 1),
            block_dim: (32, 1, 1),
            shared_mem_bytes: 0,
        };
        let r16s = |din: &DeviceBuffer<f32>,
                    _t: &DeviceBuffer<f32>,
                    dout: &mut DeviceBuffer<f32>|
         -> Result<()> {
            mod_r.fft_c2c_256_r16s(stream.as_ref(), rscfg, din, &d_w256, dout)?;
            Ok(())
        };
