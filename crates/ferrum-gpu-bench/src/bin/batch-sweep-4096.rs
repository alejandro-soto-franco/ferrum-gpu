...
fn main() -> Result<()> {
    ...
    // Same many-block correctness gate for the register four-step kernel.
    {
        ...
        println!("fft_c2c_4096_4step_reg vs radix-2: max_rel_err = {worst:.2e} -> {}", if worst<=1e-3 {"PASS"} else {"FAIL"});
        if worst>1e-3 { return Err(anyhow!("4step_reg-4096 correctness FAIL")); }
    }

    println!("N=4096 batch sweep (ratio=ferrum/cufft, <1.0 BEATS cuFFT)");
    println!("{:<9} {:<9} {:<9} {:<9} {:<10} {:<12}", "batch", "radix8", "r16s", "4step", "cufft_us", "best");
    for &b in &batches {
        ...
        let rs = |din:&DeviceBuffer<f32>, _t:&DeviceBuffer<f32>, dout:&mut DeviceBuffer<f32>| -> Result<()> {
            mod_r.fft_c2c_4096_4step_reg(stream.as_ref(), cfgr, din, &d_w4096, dout)?; Ok(()) };
        ...
        println!("{:<9} {:<9.3} {:<9.3} {:<9.3} {:<10.4} {:<12}", b, r8_r, r16_r, r4s_r, cu_us,
