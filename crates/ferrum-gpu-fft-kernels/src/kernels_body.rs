...
mod kernels {
    ...
    /// Split-layout twin of `fft_c2c_1024`. Compute identical; only global
    /// load/store changed to two coalesced f32 streams.
    #[kernel]
    pub fn fft_c2c_1024_split(
        in_re: &[f32],
        in_im: &[f32],
        twiddles: &[f32],
        mut out_re: DisjointSlice<f32>,
        mut out_im: DisjointSlice<f32>,
    ) {
        static mut BUF: SharedArray<f32, 2048> = SharedArray::UNINIT; // 1024 complex
        ...
        let block = thread::blockIdx_x() as usize;
        let tid = thread::threadIdx_x() as usize;
        let lane_off = block * N; // split: one element per complex

        {
            let mut t = tid;
            while t < N {
                unsafe {
                    BUF[2 * t] = in_re[lane_off + t];
                    BUF[2 * t + 1] = in_im[lane_off + t];
                }
                t += THREADS;
            }
        }
...

        {
            ...
            while t < N {
                let (re, im) = unsafe { (BUF[2 * t], BUF[2 * t + 1]) };
                unsafe {
                    *out_re.get_unchecked_mut(lane_off + t) = re;
                    *out_im.get_unchecked_mut(lane_off + t) = im;
                }
                t += THREADS;
            }
        }
    ...
    /// Split-layout twin of `fft_c2c_256_r16s`. On-chip algorithm is identical;
    /// only global load/store changes (two coalesced f32 streams instead of one
    /// interleaved u64 stream). Isolates the global-memory-layout variable.
    #[kernel]
        pub fn fft_c2c_256_r16s_split(in_re: &[f32], in_im: &[f32], w256: &[f32], mut out_re: DisjointSlice<f32>, mut out_im: DisjointSlice<f32>) {
        static mut BUF: SharedArray<f32, 512> = SharedArray::UNINIT;
        ...
        let blk = thread::blockIdx_x() as usize;
        let tid = thread::threadIdx_x() as usize;
        let lane_off = blk * N; // split: one element per complex

        // Coalesced load: each thread reads complex t as one u64 (re|im).
        {
            let mut t = tid;
            while t < N {
                // Split load: two fully-coalesced f32 streams (re, im).
                let re = in_re[lane_off + t];
                let im = in_im[lane_off + t];
                unsafe {
                    BUF[2 * t] = re;
                    BUF[2 * t + 1] = im;
                }
        ...
        // Split store: two fully-coalesced f32 streams (re, im).
        {
            let mut t = tid;
            while t < N {
                let (re, im) = unsafe { (BUF[2 * t], BUF[2 * t + 1]) };
                let pr = unsafe { out_re.get_unchecked_mut(lane_off + t) as *mut f32 };
                let pi = unsafe { out_im.get_unchecked_mut(lane_off + t) as *mut f32 };
                unsafe { *pr = re; *pi = im; }
                t += THREADS;
            }
        }
    ...
    /// Split-layout twin of `fft_c2c_4096_4step_reg`. Register four-step and
    /// shared transposes identical; only the global gather/scatter branches
    /// of `stage!` change to two coalesced f32 streams.
    #[kernel]
    pub fn fft_c2c_4096_4step_reg_split(in_re: &[f32], in_im: &[f32], w4096: &[f32], mut out_re: DisjointSlice<f32>, mut out_im: DisjointSlice<f32>) {
        static mut BUF: SharedArray<f32, 8192> = SharedArray::UNINIT; // 4096 complex
        ...
        macro_rules! stage {
            ($m_r:expr, $fac:expr, $do_tw:expr, $g_glob:expr, $s_glob:expr) => {{
                ...
                macro_rules! g { ($p:expr) => {{
                    let c = src_base + ($p)*NR;
                    if $g_glob {
                        // Split: two coalesced f32 streams (re, im).
                        (in_re[lane_off + c], in_im[lane_off + c])
                    } else {
                        let s = 2*sw!(c);
                        unsafe { (BUF[s], BUF[s+1]) }
                    }
                ...
                macro_rules! s { ($q:expr,$re:expr,$im:expr) => {{
                    ...
                    if $s_glob {
                        ...
                        let d = lane_off + c;
                        unsafe {
                            *(out_re.get_unchecked_mut(d) as *mut f32) = $re;
                            *(out_im.get_unchecked_mut(d) as *mut f32) = $im;
                        }
                    } else {
                        let d = 2*sw!(c);
                        unsafe { BUF[d]=$re; BUF[d+1]=$im; }
