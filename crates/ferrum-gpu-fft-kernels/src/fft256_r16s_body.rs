// N=256 radix-16, FULLY SCALARISED, 1 FFT/block (32 threads), u64-coalesced IO.
// 256 = 16x16 Stockham, 2 stages (half radix-4's shared round-trips). At large
// batch the kernel is DRAM-bound; the win over radix-4 (~1.27x plateau) is the
// fewer shared stages, and over the multi-FFT/block variant is keeping each
// warp on ONE contiguous FFT region (better coalescing).
//
// IO coalescing: cuda-oxide emits scalar generic `ld.b32` for `&[f32]` indexing
// (no ld.global, no vectorisation — CuSimd<f32,4> lowered to 4 ld.b32), giving
// only ~22% sector utilisation on the strided re/im loads. Loading each complex
// as ONE `u64` (8 B = re|im) makes consecutive threads read contiguous 8 B ->
// near-100% coalescing. re/im are recovered via f32::from_bits (the engine's
// interleaved [re,im,...] layout is little-endian: re in the low 32 bits).
//
// 32 threads: all 32 do the (coalesced) global load/store; lanes 0..15 run the
// two radix-16 butterfly stages (barriers are block-scope, outside the `if`).
// dft16 = two scalar dft8 + 8 W_16 combines, all named registers.
// Built only via `cargo oxide` (batch-sweep), not the shipped wheel.
