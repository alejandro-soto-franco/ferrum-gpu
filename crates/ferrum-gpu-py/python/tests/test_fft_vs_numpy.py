"""GPU FFT vs numpy.fft.fft on the 8-case matrix from Plan 3."""
...
@pytest.mark.parametrize(
    "log_n,batch,direction,normalize,label",
    [
...
def test_gpu_fft_matches_numpy(log_n, batch, direction, normalize, label):
    n = 1 << log_n
    inp = _input_signal(n, batch)
    expected = _reference(inp, n, batch, direction, normalize)
    out = fg.fft.fft_1d_c2c_pow2(inp, log_n=log_n, direction=direction, normalize=normalize)
    assert out.shape == inp.shape, f"{label}: shape mismatch"

    diff = out - expected
    err = float(np.max(np.abs(diff))) / max(1e-9, float(np.max(np.abs(expected))))
    assert err < 1e-4, f"{label}: max rel err {err} >= 1e-4"
